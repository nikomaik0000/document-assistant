...
## Phase 2F - 保留文件原始方向，不再強制轉直式（2026-07-20）

只改 `lib/scanner/documentScanner.ts`、`lib/scanner/manualCrop.ts` 兩支檔案。
OCR、UI、PDF 匯出、下載 JPG 皆未修改。

### 問題
自動校正／手動裁切內部都有一個「幾何啟發式自動旋轉」：只要校正結果寬度大於
高度，就強制轉成直式（假設多數文件是直式 A4）。這對身分證、健保卡、名片這類
本來就是橫式的證件／卡片是錯的——被強制轉向後文字方向不對，OCR 幾乎無法辨識。

### 修正
移除 `autoRotateToPortrait()` 這個函式（兩個檔案裡各刪一份，兩邊各自獨立實作
是先前為了不互相耦合的既有架構）。`warpPerspective` 算出來的寬高就是依實際
四個角點距離計算的結果，直接輸出，不再額外旋轉：
- 寬 > 高 → 維持橫式（身分證、健保卡、信用卡等）
- 高 > 寬 → 維持直式（A4 文件、收據等）

OCR 使用的就是這個保留原始方向的校正圖，不需要另外調整。

### 驗證：真實瀏覽器 E2E，五種情境全部測試
用合成測試照片（皆帶輕微透視角度，模擬手機拍攝），跑完整 App 流程（拖曳上傳
→ 自動校正 → 辨識文字），直接讀取畫面上校正後圖片的實際尺寸確認方向：

| 情境 | 校正後尺寸 | 方向 | OCR 結果 |
|---|---|---|---|
| A4 文件 | 470×686 | ✅ 直式 | 「A4文件測試／直式文件內容／第一段落文字說明」正確 |
| 收據 | 291×673 | ✅ 直式 | 「收據RECEIPT／ITEMA $12.00／ITEMB $8.50／TOTAL $20.50」正確 |
| 身分證 | 650×404 | ✅ 橫式 | 「NATIONAL ID CARD／NAME: TEST USER／ID NO: A123456789」正確 |
| 健保卡 | 653×420 | ✅ 橫式 | 「HEALTH INSURANCE CARD／NAME: TEST USER／CARD NO: 0012345678」正確 |
| 信用卡 | 656×413 | ✅ 橫式 | 「CREDIT CARD／1234 5678 9012 3456／VALID THRU 12/30」正確 |

五種情境全部通過，全程 **0 個 HTTP 錯誤、0 個例外**。身分證／健保卡／信用卡
三種橫式證件在修正前會被強制轉成直式導致 OCR 幾乎讀不到字，修正後維持橫式、
OCR 正確辨識。

### 驗證
- ✅ `npm install` / `npm run dev` / `npm run build` / `tsc --noEmit` /
  `eslint`：乾淨環境下皆成功
- ✅ 真實瀏覽器 E2E：五種情境（A4、收據、身分證、健保卡、信用卡）皆驗證通過
